...
$('.login').click(function(){
	$.ajax({
	    url: 'http://www.fordelivery.com.br/app/login.php?nocache=' + (new Date()).getTime(),
	    type: 'POST',
		cache:false, 
	    data: {1:1},
		crossDomain:true,
...
$.ajax({
		url: 'app/ping.php?nocache=' + (new Date()).getTime(),
		cache:false,
		dataType: 'jsonp',
		jsonp: 'callback',
		timeout: 5000,
...
$.ajax({
    url: 'app/func-mensagem.php?nocache=' + (new Date()).getTime(),
    type: 'POST',
	cache:false,
...
$.ajax({
	url: 'app/func-infotrack.php?track='+track+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$("#voltar-header").hide();
	...
	hideDeffered.promise().done(function() {
		...
		$('#radio').show("fade", { direction: "right", easing: 'easeInOutBack' }, 1000, function(){
			...
			$.ajax({
				url: 'app/func-programacao.php?nocache=' + (new Date()).getTime(),
				cache:false,
				dataType: 'jsonp',
				jsonp: 'callback',
				timeout: 10000,
...
$("#voltar-header").hide();
	...
	hideDeffered.promise().done(function() {
		if($('#programa option').size() == 1){ // Se só tiver 1 option no select, pegue o restante.
			$.ajax({
				url: 'app/func-programas.php?nocache=' + (new Date()).getTime(),
				cache:false,
				dataType: 'jsonp',
				jsonp: 'callback',
				timeout: 10000,
...
$('#titulo-header').html("<div style='margin-left: -30px;height: 35px;'>Cadastro VIP</div>");
$('.header-controls').hide();
$("#voltar-header").hide();
	e.preventDefault();
...
$("<div title='Confirmação'></div>").dialog({
   open: function(event, ui) { $(this).html('<span style="text-align:"center;">Deseja receber noficiações no aplicativo?</span>'); 
...
hideDeffered.promise().done(function() {
	...
	$('#noticias').show("fade", { direction: "right", easing: 'easeInOutBack' }, 1000,function(){
		if($("#status-noticia").val() != 1){
			...
			$.ajax({
				url: 'app/func-destaque.php?nocache=' + (new Date()).getTime(),
				cache:false,
				dataType: 'jsonp',
				jsonp: 'callback',
				timeout: 10000,
...
$.ajax({
	url: 'app/func-noticia.php?t='+t+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$.ajax({
	url: 'app/func-detalhe-noticia.php?id='+id+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$('.img_like').on('click', function() {
		...
		if(tem > -1) {
			...
			likes.splice(i, 1);
			var urllike = 'app/func-like.php?id='+id+'&t=d&nocache=';
		}
		else{
			...
			var urllike = 'app/func-like.php?id='+id+'&t=s&nocache=';
		}
...
$.ajax({
	url: 'app/func-podcast.php?nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$.ajax({
	url: 'app/func-buscanoticia.php?texto='+noticia+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
...
$('#resultado').html('<div class="sidebar-divider side-resultado"><img src="imagens/loader-busca.gif" style="margin:0 auto;"/></div>');	
$.ajax({
	url: 'app/func-buscapodcast.php?termo='+termo+'&nocache=' + (new Date()).getTime(),
	cache:false,
	dataType: 'jsonp',
	jsonp: 'callback',
	timeout: 10000,
